  ...
  return (
    <div>
      <div className="flex items-center justify-between mb-8">
        <div>
          <h1 className="text-2xl font-extrabold text-white">Tour Guides</h1>
          <p className="text-gray-400 text-sm mt-1">{items.length} guide{items.length !== 1 ? "s" : ""}</p>
        </div>
        <Link href="/admin/tour-guides/new" className="flex items-center gap-2 bg-[#01b7f2] hover:bg-[#0299cc] text-white font-bold px-4 py-2.5 rounded-lg text-sm transition-colors">
      ...
      {items.length === 0 ? (
        <div className="text-center py-20 bg-[#1e293b] rounded-xl border border-slate-700">
          ...
          <p className="text-gray-400 mb-4">No tour guides added yet</p>
          <Link href="/admin/tour-guides/new" className="btn-primary">Add First Guide</Link>
        </div>
      ) : (
        <>
              ...
              <tbody>
                {paged.map((item, i) => (
                  <tr key={item._id} className="border-b border-slate-800 hover:bg-slate-800/50 transition-colors">
                    ...
                    <td className="px-5 py-3">
                      <div className="flex items-center gap-3">
                        {item.image ? (
                          ...
                          <div className="w-10 h-10 rounded-full bg-[#0A65AB]/20 flex items-center justify-center flex-shrink-0 text-base">🧭</div>
                        )}
                        <div>
                    ...
                    <td className="px-5 py-3 text-gray-400 hidden md:table-cell">{item.phone || "—"}</td>
                    <td className="px-5 py-3 text-gray-300 hidden lg:table-cell">{item.experience ? `${item.experience} yr${item.experience !== 1 ? "s" : ""}` : "—"}</td>
                    ...
                    <td className="px-5 py-3">
                      <span className={`text-xs px-2 py-0.5 rounded-full font-medium ${item.featured ? "bg-[#0A65AB]/20 text-[#0A65AB]" : "bg-slate-700 text-gray-400"}`}>
                        {item.featured ? "Yes" : "No"}
                      </span>
                    </td>
                    <td className="px-5 py-3 text-right">
                      <div className="flex items-center justify-end gap-2">
                        <Link href={`/admin/tour-guides/${item._id}/edit`} className="text-gray-400 hover:text-[#01b7f2] transition-colors p-1.5 hover:bg-slate-700 rounded-lg">
                          <Pencil size={14} />
                        </Link>
