    ...
    <section className="section-padding bg-white">
      <div className="container-custom">
        ...
        <div className="grid md:grid-cols-3 gap-8 relative">
          {/* Connector line */}
          <div className="hidden md:block absolute top-10 left-1/4 right-1/4 h-0.5 bg-gradient-to-r from-[#01b7f2] via-[#01b7f2] to-[#01b7f2] opacity-20 z-0" />

          {steps.map(({ step, icon: Icon, title, desc }, i) => (
            <div key={step} className="relative z-10 text-center">
              {/* Step number circle */}
              <div className="relative inline-flex mb-6">
                <div className="w-20 h-20 bg-[#0A65AB] rounded-full flex items-center justify-center shadow-xl">
                  <Icon size={28} className="text-[#01b7f2]" />
                </div>
                <div className="absolute -top-2 -right-2 w-7 h-7 bg-[#01b7f2] rounded-full flex items-center justify-center text-white text-xs font-black">
                  {i + 1}
                </div>
              </div>

              <div className="text-xs font-bold text-[#01b7f2] mb-2 tracking-widest">{step}</div>
              <h3 className="font-extrabold text-[#0A65AB] text-xl mb-3">{title}</h3>
